...
/**
 * Adapter implements Adapter interface from nephele module using Drive files
 */
export class Adapter implements INepheleAdapter {
   ...
   */
  async isAuthorized(url: URL, method: string, baseUrl: URL, user: INepheleUser): Promise<boolean> {
  ...
  async getResource(url: URL, baseUrl: URL): Promise<INepheleResource> {
    ...
    const resource = new Resource(this.nephele, {
      adapter: this,
      baseUrl: baseUrl,
    ...
    if (!(await resource.exists())) {
      throw new this.nephele.ResourceNotFoundError("Resource not found");
    }
    return resource;
  }
  /**
  ...
  async newResource(url: URL, baseUrl: URL): Promise<INepheleResource> {
    ...
    if (pathname_arr.length == 0) {
      throw new this.nephele.BadGatewayError("This resource is not managed by this adapter");
    }
    const resource = new Resource(this.nephele, {
      adapter: this,
      baseUrl: baseUrl,
...
export const getDriveExecutionContext = (url: URL): DriveExecutionContext => ({
  ...
  url: url.href,
  method: "from_webdav", // cannot retrieve method from url
  reqId: "from_webdav", // cannot retrieve request Id from url
