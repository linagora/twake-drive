...
Drive declares this service in its manifest:
...
- Works for files by default.
- Folders are disabled by default (`allowFolder: false`).
...
There is no `{ files, errors }` envelope.
