...
| T9 | `[T1.intra avec image]` *(cellule avec image)* | ✅ copie après tableau ; original (avec image) non touché → image préservée | ⚠️ clone via ToJSON → image PERDUE à la sauvegarde | ⚠️ · **L#8** · VALIDÉ LIVE 2026-06-26 (build .5, fixture table-image). Insert ✅ : original non reconstruit, son média survit (after.docx a:blip=2). Replace structurel ❌ : l'image en cellule round-trip live (rendu OK) mais est PERDUE dans le .docx sauvé (clone reconstruit via ToJSON perd le fill image — caveat Copy-vs-ToJSON ; original détruit emporte son média). NB intra-cellule pure-image → extraction VIDE. Fix tenté (reconstructTable->Copy si image, build .6) INEFFICACE : la cellule image est reconstruite par replaceCellContent via le marqueur ![IMG], et l'image re-insérée vit mais n'est pas persistée (le tableau original, propriétaire de media/image1.png, est détruit par le replace structurel -> média orphelin droppé à la save). Cause = gestion média OO, hors portée plugin simple. Goldens corpus/T9/{insert(pass),replace(xfail)}. |
...
- **L#2** — Replace cross-frontière (¶↔cellule, ou deux tableaux) : **le document résultant est CORRECT** — cellules remplacées in-place + ¶ remplacés in-place, contenu non sélectionné intact. Le seul manque = la **post-sélection** : le curseur retombe collapsed (pas de span sur le contenu remplacé), car l'API OO ne *ré-sélectionne* pas à cheval sur une frontière de cellule. **VÉRIFIÉ LIVE 2026-06-26 (build .7)** sur T4/T5/T6 : la *mise en place* de la sélection cross-frontière, elle, marche (driver `ExpandTo()`+`Select()`, twin du chemin prod) — c'était la limite « driver » antérieure, désormais levée. xfail = cosmétique (curseur), pas de corruption. Goldens `corpus/T{4,5,6}/{insert,replace}`.
