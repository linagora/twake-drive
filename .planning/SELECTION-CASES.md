...
| T2a | `[T1.cells,T1.cells]` *(cellules partielles, sans fusion)* | ✅ copie réduite du tableau insérée après le tableau | ✅ in-place modifyOriginalTableCells; cellules non sél. et vides intactes | ✅ · Replace in-place VALIDÉ LIVE 2026-06-26 ; Insert NON validé live (le harnais injectAtSelection court-circuite l'état snapshots d'extraction, hasSnap=false → no-op observé NON concluant) → à vérifier en IHM réelle |
| T2b | `[T1.cells,T1.cells]` *(cellules partielles, fusion H traversée)* | ⚠️ clone complet (jamais RemoveColumn) | ✅ in-place — identique à T2a (round-trip (r,c), S2) | ⚠️ · Replace in-place VALIDÉ LIVE 2026-06-26 (gridSpan préservé, cellsPerRow=[3,3,3,2]) ; Insert xfail (corruption RemoveColumn Q4, sonde 2026-06-17) NON re-vérifié live ce flux ; aperçu GFM désaligné cosmétique |
| T2c | `[T1.cells,T1.cells]` *(cellules partielles, fusion V traversée)* | ⚠️ clone complet (jamais RemoveRow) | ✅ in-place — identique à T2a (maître édité, continuation vide jamais touchée, S3/S4) | ⚠️ · Replace in-place VALIDÉ LIVE 2026-06-26 (continuation (2,0) jamais touchée, cellsPerRow=[3,3,3,2]) ; Insert xfail (Q4) NON re-vérifié live ce flux ; S3 mismatch UX ; trou détection full S5 |
| T3 | `[T1.full,T1.full]` *(tableau entier englobant)* | ✅ clone complet inséré après le tableau | ✅ clone + InsertContent; post-sélection OK | ✅ · Replace in-place VALIDÉ LIVE 2026-06-26 ; Insert NON validé live (même réserve harnais que T2a : snapshots non peuplés ; NB : insert table à un curseur ¶ doc-level FONCTIONNE en harnais) → à vérifier en IHM réelle |
...
- **Insert de contenu table : no-op OBSERVÉ en harnais, NON concluant (1ʳᵉ conclusion « bug » RÉTRACTÉE).** L'injection multi-cellules via `injectAtSelection` laisse le doc inchangé — MAIS le harnais **court-circuite l'état snapshots d'extraction** (`hasSnap=false` au moment de l'inject ; en prod les snapshots arrivent par relais postMessage de l'hôte). Donc le harnais **n'est pas un banc fidèle pour l'Insert tableau**, et le no-op n'est pas une preuve de bug prod. NB : insérer une table à un **curseur ¶ doc-level** fonctionne en harnais ; et le code prod a déjà une logique de repositionnement curseur-après-tableau (`code.js:1773`). → **Verdict Insert T2a/T3 = ✅ d'origine maintenu, à VÉRIFIER EN IHM RÉELLE** avant tout fix. Détail + correction : `REVIEW-LOG.md` (2026-06-26).
