...
| T5 | `[T1.C(1,0),P2@mid]` *(tableau -> finit après)* | ✅ après la fin de sélection: copie réduite tête + ¶/tableaux suivants (cas 2c) | ⚠️ cellules in-place + InsertContent par ¶ | ⚠️ · **L#2** · Insert RE-CAPTURÉ 2026-06-28 (build .10) après fix T7 : place la réponse à la FIN DE LA SÉLECTION (P2@mid, scinde Outro) conforme à expected — l'ancien golden (build .7) l'insérait à tort 'après le tableau'. docElementOrder=[Intro,T1,Outro-tête,clone(GG/DD),Tail edit,Outro-queue]. Replace reste xfail L#2 (post-sélection collapsed, doc correct). Goldens corpus/T5/{insert(re-capturé),replace}. |
