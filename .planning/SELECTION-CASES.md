...
| T2a | `[T1.cells,T1.cells]` *(cellules partielles, sans fusion)* | ⚠️ copie réduite du tableau insérée après le tableau | ✅ in-place modifyOriginalTableCells; cellules non sél. et vides intactes | ⚠️ · Insert = NO-OP confirmé live 2026-06-26 (curseur replié dans la cellule → clone jamais posé) ; Replace in-place validé live |
| T2b | `[T1.cells,T1.cells]` *(cellules partielles, fusion H traversée)* | ⚠️ clone complet (jamais RemoveColumn) | ✅ in-place — identique à T2a (round-trip (r,c), S2) | ⚠️ · Insert = NO-OP live 2026-06-26 (curseur replié, ANTÉRIEUR à toute réduction → Q4/RemoveColumn JAMAIS atteint par ce flux) ; Replace in-place validé live (gridSpan préservé) ; aperçu GFM désaligné cosmétique |
| T2c | `[T1.cells,T1.cells]` *(cellules partielles, fusion V traversée)* | ⚠️ clone complet (jamais RemoveRow) | ✅ in-place — identique à T2a (maître édité, continuation vide jamais touchée, S3/S4) | ⚠️ · Insert = NO-OP live 2026-06-26 (curseur replié, antérieur à RemoveRow) ; Replace in-place validé live (continuation (2,0) jamais touchée, cellsPerRow=[3,3,3,2]) ; S3 mismatch UX ; trou détection full S5 |
| T3 | `[T1.full,T1.full]` *(tableau entier englobant)* | ⚠️ clone complet inséré après le tableau | ✅ clone + InsertContent; post-sélection OK | ⚠️ · Insert = NO-OP confirmé live 2026-06-26 (même cause que T2a ; or l'insert table à un curseur ¶ doc-level FONCTIONNE → bug = curseur piégé en cellule) ; Replace in-place validé live |
