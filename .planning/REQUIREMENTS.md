...
- [x] **HARDEN-02**: Un corpus de régression de réponses malformées + tests de cas limites passe au vert ; le taux de `fellBack` code-fence est re-mesuré, et le défaut du flag `response_format` (prompt-based vs structured-output) est décidé et documenté
