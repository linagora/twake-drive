...
- **2026-06-26 — Driver étendu (cross-cellule + table entière) + axe REPLACE tableaux VALIDÉ LIVE ; INSERT table = NO-OP découvert.** Build `2026-06-26.2`. Le driver accepte désormais `T<n>.C(r1,c1)@..T<n>.C(r2,c2)@` (range cross-cellule, même table) et `T<n>.full` (table entière) ; `selSpecSupported` remplace la garde intra-only ; les deux résolveurs jumeaux gagnent une branche `ExpandTo(1er ¶ cellule début → dernier ¶ cellule fin)`. **Code frais chargé sans redémarrer oo-dev** (contexte isolé neuf). Validations live :
  ...
  - **⚠️ INSERT de contenu table = NO-OP** (table-plain T2a/T3 ET table-merged full) : document **strictement inchangé** (confirmé écran `merged-full-insert.png`, pas de table imbriquée — `dumpState` ignore pourtant le nested, vérif visuelle faite). Cause : le handler insert (652-686) cherche l'hôte parmi les ¶ **top-level** ; sur une sélection table `hostPara=null` → curseur replié **dans la dernière cellule** → `InsertContent` du clone de table n'y pose rien. **Contredit le ✅ spec pour T2a/T3 Insert.** ⚠️ À confirmer : ma sélection synthétique (`ExpandTo` de ranges texte) n'est peut-être pas une *vraie* sélection de cellules (drag UI) — l'INSERT « copie après tableau » dépend peut-être de ce type de sélection. **À trancher avec Ben** (test sélection UI réelle vs bug confirmé). NB : pour T2b/c, ce no-op survient **avant** tout `RemoveColumn` → la corruption Q4 n'est pas atteinte par ce flux.
