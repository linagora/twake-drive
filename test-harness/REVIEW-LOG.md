...
- **2026-06-26 (T9 — tentative de fix ÉCHOUÉE, revertée) — bug plus profond qu'estimé.** Fix tenté (build `.6`, NON commité, **REVERTÉ** → code.js reste `.5`) : faire cloner `reconstructTable` via `Copy()` (au lieu de FromJSON/ToJSON) quand le tableau a une image. **Inefficace** : après `.6`, le replace structurel perd toujours l'image (`a:blip=0`, media=[]). Raison : la cellule image n'est PAS conservée depuis le clone — `replaceCellContent` la **reconstruit** à partir du marqueur `![IMG:scribe-img-0]` (drawingIndex Copy), donc le choix FromJSON/Copy dans `reconstructTable` est hors-chemin. La vraie cause : sur un **replace structurel**, le tableau original (propriétaire de `media/image1.png`) est **détruit** → le média devient orphelin et l'image re-insérée (qui le référence) est droppée à la sauvegarde. C'est de la **gestion de relations média OO**, pas un one-liner plugin. → T9 Replace reste **xfail** ; un vrai fix demande d'enquêter sur la duplication du média à la Copy/InsertContent (chantier dédié). Insert reste ✅ (original gardé → média vivant).
- **2026-06-26 (T9 image en cellule — bug PERSISTANCE trouvé) — Insert ✅ / Replace ❌.** Générateur étendu (PNG embarqué : media + relationship + `<w:drawing>`, conditionnel donc les autres fixtures restent byte-identiques) → fixture `table-image.docx` (image en cellule (0,0)). OO la parse/rend (carré rouge, drawingsInCells=1). Résultats : **Insert (T1.full)** = copie après tableau, **original non touché → image préservée** (after.docx `a:blip=2`, media OK) ✅. **Replace structurel (T1.full)** = image **PERDUE à la sauvegarde** ❌ : live l'éditeur la rend (rouge) mais after.docx (forcesave ×3) n'a NI `a:blip` NI media → le clone reconstruit via **ToJSON perd le fill image** (caveat Copy-vs-ToJSON), et l'original détruit emporte son média. **Discriminé** : pristine round-trip OK (`a:blip=2`) + replace partiel in-place (cellule image non touchée) préserve l'image → bug localisé au chemin `reconstructTable`/ToJSON en replace structurel. Fix proposé (non fait) : `reconstructTable` doit utiliser `Copy()` (pas FromJSON/ToJSON) si le tableau contient une image. Aussi : **intra-cellule pure-image → extraction VIDE** (rien envoyé au LLM). Goldens `corpus/T9/{insert(pass),replace(xfail)}`. cases.csv T9 : insert=pass, replace=xfail.
- **2026-06-26 (T8 re-statué ✅ + goldens) — cellule multi-¶ : L#3 INFIRMÉ.** T8 était `xfail` sur la crainte L#3 « block mode déborde hors cellule ». Test live (build `.5`, cellule (1,1) multi-¶ Delta/Delta2, contenu injecté multi-¶ `AAA\n\nBBB` / `CCC\n\nDDD`) : **zéro débordement** (blockCount=3, tableau intact), multi-¶ correctement injecté in-cellule, 2e ¶ non sélectionné (Delta2) **préservé**. Seul résidu = ¶ vide cosmétique (L#7, commun top-level). → **T8 insert+replace = ✅** ; **L#3 marqué INFIRMÉ** dans la légende. Goldens `corpus/T8/{insert,replace}` (procédure v4). **Axe tableau testable : T1/T2a/T2b/T2c/T3/T8 tous ✅ + goldens.** (Restent hors-scope-session : T4/T5/T6 cross-boundary replace=xfail by-design L#2 ; T9 image = fixture manquante ; T11 raisonnement ; T13 unsupported.)
- **2026-06-26 (GOLDENS tableaux capturés + 3e bug isSimpleInline) — corpus `corpus/T{1,2a,2b,2c,3}/{insert,replace}` (10 bundles, procédure v4).** Capture des goldens de l'axe tableau, build `2026-06-26.5`. Procédure v4 (vs v3) : (a) **pas de `grabFocus`** (évite le vol de focus signalé par Ben) → `before.png` = doc pristine avec ¶ marks, sélection documentée dans `capture.json` ; (b) pour les cas multi-cellules, **`extractSelection` AVANT `injectAtSelection`** (peuple `tableDocIndices`/snapshots via le cabling test-only) ; (c) `after.docx` via forcesave+download avec **verify-retry par token unique** (tolère `error:4` = autosave déjà à jour) ; (d) `before.png` pristine capturé 1×/fixture et copié (identique sans grabFocus). Tooling recréé dans le scratchpad (`assemble.py` curl-based, `normalize.mjs`). **3e instance du bug `isSimpleInline` trouvée pendant la capture** : T3 full **Replace** vidait juste la 1ère cellule (clone-table classé inline) → fix `.5` (frère du `.3`/insert), commit `ccb38a31c`. Verdicts goldens : T1/T2a/T2b/T2c/T3 × insert/replace **tous pass**, `model.json` = sortie live. NB `selection` post-inject = `block:-1` (curseur en cellule non mappé par dumpState, limite connue, documentée). **Découverte clés harnais** : `Asc.scope` ne survit pas hors d'un même callCommand ; `window` inaccessible dans le sandbox callCommand → instrumenter via la valeur de retour. Onglets OO ouverts en contexte par défaut + `background:true` (même fenêtre, pas de premier-plan) ; code frais chargé en contexte défaut neuf (cache plugin `immutable` ⇒ reload ne suffit pas).
- **2026-06-26 (§4bis#2 IMPLÉMENTÉ) — Insert tableau fusionné = clone COMPLET (build `2026-06-26.4`, commit `34e6ade86`).** Décision §4bis #2 (verrouillée) enfin codée : `tableHasMerge(table)` scanne le ToJSON lossless (formats sondés : `"gridSpan":N` N≥2 = fusion H ; `"vMerge":"restart|continue"` = fusion V — émis UNIQUEMENT pour cellules fusionnées). Dans le chemin Insert partiel, si fusion → on garde le **clone complet** (cellules sélectionnées modifiées) et on **skip `pendingTableReductions`** (RemoveColumn/RemoveRow corrompent les spans, Q4). **Validé live** : T2c partiel V-region Insert → copie = clone complet `cpr=[3,3,3,2]`, continuation `(2,0)` vide, H-span intact, ZÉRO corruption (`t2c-insert-fullclone.png`) ; régression vérifiée : T2a non-fusionné réduit toujours à `[[AA,BB]]`. → **T2b/c Insert = ✅** (étaient ⚠️). **Axe tableau Insert+Replace désormais tout ✅ : T1/T2a/T2b/T2c/T3.**
- **2026-06-26 (RÉSOLUTION) — Insert tableau : VRAI bug prod trouvé ET corrigé (build `2026-06-26.3`).** Après avoir rendu le harnais fidèle (cabler le relais snapshots, cf §Option A : l'extraction renvoie `tableSnapshots` dans le JSON quand `__scribeTestForce`, gated/inert en prod), le no-op a persisté MÊME avec `hasSnap=true` → ce n'était donc pas qu'un artefact. Diagnostic via le **seul canal fiable hors callCommand = la valeur de retour** (window/Asc.scope inaccessibles depuis le sandbox callCommand) : le clone était bien bâti (`cloneKeys:["0=obj"]`), `content.length=1`, curseur repositionné après le tableau (`afterPos=58`) — mais rien ne s'insérait. **Cause = bug prod réel** : `insSimpleInline` testait `blocks[0].type==='paragraph'`, or `blocks[0]` est le **¶ placeholder `SCRIBE-TABLE-0`**, pas le `content[0]` réel (un TABLEAU substitué). Quand la réponse est UNIQUEMENT un tableau (T2a/T3), ça classait l'insert en **inline** → `InsertContent([table], true)` à un curseur replié = **no-op silencieux**. **Fix** (`code.js`, ~1 ligne) : exclure le cas où `content[0].GetClassType()==='table'`. **Validé live** (build .3, pristine) : T3 full → copie complète après le tableau, original intact (`t3-insert-fixed.png`) ; T2a partiel → copie réduite `[[AA,BB]]` après ; T2b H-row → copie réduite, fusion NON corrompue. Verdicts `cases.csv` : **T2a/T3 Insert = ✅** ; **T2b/c Insert = ⚠️** (no-op corrigé mais §4bis#2 « fusion → clone complet, jamais RemoveColumn/Row » toujours non implémenté → une sélection à retrait de colonne via span H peut encore corrompre, Q4). Commit `619d198fb`. **Leçons** : (a) `Asc.scope` ne survit pas callCommand→callback ni entre callCommands → instrumenter via la valeur de retour ; (b) le harnais `/example/` n'a pas l'hôte Cozy Drive → snapshots à câbler pour tester l'Insert tableau.
- **2026-06-26 (CORRECTION — désormais RÉSOLUE, voir ci-dessus) — le « INSERT table = no-op confirmé bug » avait été RÉTRACTÉ comme non concluant.** Deux causes de non-fiabilité découvertes après coup : (1) le harnais `injectAtSelection` **court-circuite l'état snapshots d'extraction** — au moment de l'inject `hasSnap=false` (sonde fiable sur `window` du plugin), alors qu'en prod les snapshots ToJSON arrivent par relais postMessage de l'hôte (`code.js:2097-2100`) ; (2) mon instrumentation interne était **inexploitable** : `Asc.scope` **ne persiste PAS d'un `callCommand` à un autre** (test direct : write puis read séparé → `UNSET`), donc tous les « sinks » lus indiquaient null par artefact, pas par non-exécution. Conséquence : impossible de conclure sur la cause du no-op avec ce harnais. De plus le code prod a **déjà** une logique de repositionnement curseur-après-tableau (`code.js:1773-1794`), donc l'hypothèse « curseur piégé » n'est pas le manque supposé. **Actions** : fix prod `2026-06-26.3` (repositionnement curseur — redondant et inopérant) **REVERTÉ** (working tree revenu à `2026-06-26.2`) ; verdicts `cases.csv` T2a/T3 Insert remis à **✅ d'origine** avec réserve « non validé live, à vérifier en IHM réelle » ; T2b/c Insert = **xfail (Q4)** restauré (la sonde RemoveColumn de 2026-06-17 reste la seule preuve). **Le harnais actuel ne teste pas fidèlement l'Insert tableau** (il faut soit l'IHM réelle, soit câbler le relais snapshots dans le harnais). Les résultats **Replace in-place restent valides** (chemin in-place, sans dépendance aux snapshots).
