...
const readTokenAuthor = async client => {
  try {
    ...
    logger.warn(`Cannot resolve editor author from token: ${error}`)
  }
...
/**
 * Resolves the name to attribute a document editor's contribution to — the PDF
 * annotation author, the Excalidraw live cursor, and so on. Editor-agnostic on
 * purpose, so every editor shares one source of truth.
 *
 ...
 *   absent for an anonymous link share, in which case the author stays undefined
 *   and the caller decides on a default.
 *
 * The instance settings are not readable through a public sharecode, so that
 * query is skipped in public mode (it would otherwise leak the owner's name to
 * every anonymous guest).
 *
 * @param {{ isPublic?: boolean }} [options]
 * @returns {{ author: string|undefined, isLoading: boolean }}
 */
export const useEditorAuthor = ({ isPublic = false } = {}) => {
