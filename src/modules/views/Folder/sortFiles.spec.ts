...
describe('sortFiles', () => {
  ...
  it('resolves each updated timestamp once before sorting', () => {
    const getFileLastUpdatedAtSpy = jest.fn(getFileLastUpdatedAt)

    sortFiles(
      [oldestFile, folder, newestFile],
      { attribute: 'updated_at', order: 'desc' },
      { getFileLastUpdatedAt: getFileLastUpdatedAtSpy }
    )

    expect(getFileLastUpdatedAtSpy).toHaveBeenCalledTimes(3)
  })
