...
function getLastUpdatedComparator(
  order: string,
  timestamps: ReadonlyMap<IOCozyFile, number | null>
): FileComparator {
  return (first, second) => {
    const firstTimestamp = timestamps.get(first) ?? null
    const secondTimestamp = timestamps.get(second) ?? null
...
export function sortFiles(
  ...
  let resolvedComparator: FileComparator
  if (attribute === 'updated_at') {
    const timestamps = new Map(
      files.map(file => [file, getTimestamp(getFileLastUpdatedAt(file))])
    )
    resolvedComparator = getLastUpdatedComparator(order, timestamps)
  } else {
    resolvedComparator = getFileComparator(order, attribute)
  }
