...
import { DEFAULT_SORT, SORTABLE_ATTRIBUTES } from '@/config/sort'
...
const buildFolderQuery = (dirId, type, as, limit, sortOrder = DEFAULT_SORT) => {
  const validSortOrder = SORTABLE_ATTRIBUTES.some(
    ({ attr }) => attr === sortOrder.attribute
  )
    ? sortOrder
    : DEFAULT_SORT
    ...
    definition: () =>
      Q(FILES_DOCTYPE)
        .where({
          dir_id: dirId,
          type,
          [validSortOrder.attribute]: { $gt: null }
        })
        .partialIndex({
          _id: {
            $nin: [TRASH_DIR_ID]
        ...
        .sortBy([
          { dir_id: validSortOrder.order },
          { type: validSortOrder.order },
          { [validSortOrder.attribute]: validSortOrder.order }
        ])
        .limitBy(limit),
    options: {
      as: getQueryAlias(as, validSortOrder),
